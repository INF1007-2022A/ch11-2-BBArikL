...
class Spell:
	...
	# TODO: __init__
	def __init__(self, name: str, power: int, mp_cost: int, min_level: int):
		self.name = name
		self.power = power
...
class Magician(Character):
	...
	@spell.setter
	def spell(self, val: Spell):
		if val is None:
			val = Spell("Nothing", 0, 0, 0)
		if val.min_level > self.level:
			raise ValueError("Magic spell too high level!")
		self.__spell = val
	...
	def will_use_spell(self):
		return self.using_magic

	def _compute_magical_damage(self):
		return self.compute_damage_output(
